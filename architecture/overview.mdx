...
```mermaid
graph TB
    subgraph CLI["Mobile Stack CLI"]
        A[cli.ts<br/>Commander] --> B[create-app.ts<br/>Logic]
        B --> C[EJS Templates<br/>Processor]
    end
    
    C -->|Generates| D[Mobile App]
    
    subgraph App["Generated Mobile App"]
        subgraph Screens["App Screens - Expo Router"]
            E["(auth)/login.tsx<br/>Google OAuth"]
            F["(app)/index.tsx<br/>Home/Dashboard"]
            G["(app)/settings.tsx<br/>AI Config"]
        end
        
        subgraph Services["Core Services"]
            H[AuthContext<br/>Session mgmt]
            I[ThemeContext<br/>Light/Dark mode]
            J[llmService<br/>AI models]
            K[supabase<br/>Database client]
        end
    end
    
    subgraph External["External Services"]
        L[Supabase<br/>DB + Auth]
        M[OpenAI<br/>Gemini<br/>Ollama]
